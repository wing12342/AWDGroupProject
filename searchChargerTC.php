...
if (!empty($district)) {
    ...
    $queryParts[] = "NAME_OF_DISTRICT_COUNCIL_DISTRICT_TC IN ($districtPlaceholders)";
...
if (!empty($location)) {
    ...
    $queryParts[] = "LOCATION_TC IN ($locationPlaceholders)";
    $params = array_merge($params, $location);
}

if (!empty($addresse)) {
    $addressPlaceholders = implode(',', array_fill(0, count($addresse), '?'));
    $queryParts[] = "ADDRESS_TC IN ($addressPlaceholders)";
